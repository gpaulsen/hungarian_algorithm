...
typename std::enable_if<
    !std::is_convertible<Container<T, Args...>, std::string>::value &&
...
/* Ensure that the matrix is square by the addition of dummy rows/columns if
 * necessary */
template <typename T> void pad_matrix(std::vector<std::vector<T> > &matrix) {
 ...
 * every element in its row. For each col of the matrix, find the smallest
 * element and subtract it from every element in its col. Go to Step 2. */
template <typename T>
 ...
 * In this step, we introduce the mask matrix M, which in the same dimensions as
 * the cost matrix and is used to star and prime zeros of the cost matrix.  If
 * M(i,j)=1 then C(i,j) is a starred zero,  If M(i,j)=2 then C(i,j) is a primed
 * zero.  We also define two vectors RowCover and ColCover that are used to
 * "cover" the rows and columns of the cost matrix. In the nested loop (over
 * indices i and j) we check to see if C(i,j) is a zero value and if its column
 * or row is not already covered.  If not then we star this zero (i.e. set
 * M(i,j)=1) and cover its row and column (i.e. set R_cov(i)=1 and C_cov(j)=1).
 * Before we go on to Step 3, we uncover all rows and columns so that we can use
 * the cover vectors to help us count the number of starred zeros. */
template <typename T>
void step2(const std::vector<std::vector<T> > &matrix,
           std::vector<std::vector<int> > &M, std::vector<int> &RowCover,
 ...
 * Go to DONE, otherwise, Go to Step 4. Once we have searched the entire cost
 * matrix, we count the number of independent zeros found.  If we have found
 * (and starred) K independent zeros then we are done.  If not we procede to
 * Step 4.*/
void step3(const std::vector<std::vector<int> > &M, std::vector<int> &ColCover,
           int &step) {
  int sz = M.size();
  int colcount = 0;
...
template <typename T>
void find_a_zero(int &row, int &col, const std::vector<std::vector<T> > &matrix,
...
void step4(const std::vector<std::vector<T> > &matrix,
           std::vector<std::vector<int> > &M, std::vector<int> &RowCover,
           std::vector<int> &ColCover, int &path_row_0, int &path_col_0,
           int &step) {
  int row = -1;
  int col = -1;
  bool done = false;
 ...
 * all primes and uncover every line in the matrix.  Return to Step 3.  You may
 * notice that Step 5 seems vaguely familiar.  It is a verbal description of the
 ...
 * without altering any stars, primes, or covered lines. Notice that this step
 * uses the smallest uncovered value in the cost matrix to modify the matrix.
 * Even though this step refers to the value being found in Step 4 it is more
 * convenient to wait until you reach Step 6 before searching for this value. It
 * may seem that since the values in the cost matrix are being altered, we would
 * lose sight of the original problem. However, we are only changing certain
 * values that have already been tested and found not to be elements of the
 * minimal assignment.  Also we are only changing the values by an amount equal
 * to the smallest value in the cost matrix, so we will not jump over the
 * optimal (i.e. minimal assignment) with this change. */
template <typename T>
...
typename std::enable_if<std::is_integral<T>::value,
                        T>::type // Work only on integral types
...
int main() // example of usage
{
  ...
  list<list<int> > matrix{
      {85, 12, 36, 83, 50, 96, 12, 1},  {84, 35, 16, 17, 40, 94, 16, 52},
      {14, 16, 8, 53, 14, 12, 70, 50},  {73, 83, 19, 44, 83, 66, 71, 18},
      {36, 45, 29, 4, 61, 15, 70, 47},  {7, 14, 11, 69, 57, 32, 37, 81},
      {9, 65, 38, 74, 87, 51, 86, 52},  {52, 40, 56, 10, 42, 2, 26, 36},
      {85, 86, 36, 90, 49, 89, 41, 74}, {40, 67, 2, 70, 18, 5, 94, 43},
      {85, 12, 36, 83, 50, 96, 12, 1},  {84, 35, 16, 17, 40, 94, 16, 52},
      {14, 16, 8, 53, 14, 12, 70, 50},  {73, 83, 19, 44, 83, 66, 71, 18},
      {36, 45, 29, 4, 61, 15, 70, 47},  {7, 14, 11, 69, 57, 32, 37, 81},
      {9, 65, 38, 74, 87, 51, 86, 52},  {52, 40, 56, 10, 42, 2, 26, 36},
  ...
  tests.push_back({{25, 40, 35}, {40, 60, 35}, {20, 40, 25}});

  tests.push_back({{64, 18, 75}, {97, 60, 24}, {87, 63, 15}});
